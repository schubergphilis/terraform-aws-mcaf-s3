...
| <a name="input_replication_configuration"></a> [replication\_configuration](#input\_replication\_configuration) | Bucket replication configuration settings | <pre>object({<br>    iam_role_arn = string<br>    rules = map(object({<br>      id                 = string<br>      dest_bucket        = string<br>      dest_storage_class = string<br>    }))<br>  })</pre> | `null` | no |
