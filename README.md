...
| <a name="input_replication_configuration"></a> [replication\_configuration](#input\_replication\_configuration) | Bucket replication configuration settings, specify the rule keys as integers as these are used to determine the priority of the rules in case of conflict | <pre>object({<br>    iam_role_arn = string<br>    rules = map(object({<br>      id                 = string<br>      dest_bucket        = string<br>      dest_storage_class = string<br>    }))<br>  })</pre> | `null` | no |
