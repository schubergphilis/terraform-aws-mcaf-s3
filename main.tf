locals {
  ...
  versioning                = var.versioning ? { create = true } : {}
}
...
resource "aws_s3_bucket_lifecycle_configuration" "default" {
  ...
  rule {
    ...
    status = lookup(var.lifecycle_rule[count.index], "status", "Enabled")

    dynamic "abort_incomplete_multipart_upload" {
      for_each = length(
        keys(lookup(var.lifecycle_rule[count.index], "abort_incomplete_multipart_upload", {}))
      ) == 0 ? [] : [lookup(var.lifecycle_rule[count.index], "abort_incomplete_multipart_upload", {})]
    ...
    dynamic "expiration" {
      for_each = length(
        keys(lookup(var.lifecycle_rule[count.index], "expiration", {}))
      ) == 0 ? [] : [lookup(var.lifecycle_rule[count.index], "expiration", {})]
    ...
    dynamic "noncurrent_version_expiration" {
      for_each = length(
        keys(lookup(var.lifecycle_rule[count.index], "noncurrent_version_expiration", {}))
      ) == 0 ? [] : [lookup(var.lifecycle_rule[count.index], "noncurrent_version_expiration", {})]
    ...
    dynamic "noncurrent_version_transition" {
      for_each = length(
        keys(lookup(var.lifecycle_rule[count.index], "noncurrent_version_transition", {}))
      ) == 0 ? [] : [lookup(var.lifecycle_rule[count.index], "noncurrent_version_transition", {})]
    ...
    dynamic "transition" {
      for_each = length(
        keys(lookup(var.lifecycle_rule[count.index], "transition", {}))
      ) == 0 ? [] : [lookup(var.lifecycle_rule[count.index], "transition", {})]
